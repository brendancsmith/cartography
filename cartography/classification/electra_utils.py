...
from transformers import Trainer, EvalPrediction, TrainerCallback, TrainingArguments, TrainerState, TrainerControl
...
def prepare_dataset_nli(examples, tokenizer: Tokenizer, max_seq_length=None):
...
def prepare_train_dataset_qa(examples, tokenizer: Tokenizer, max_seq_length=None):
...
def prepare_validation_dataset_qa(examples, tokenizer: Tokenizer):
    questions = [q.lstrip() for q in examples["question"]]
    max_seq_length = tokenizer.model_max_length
    tokenized_examples = tokenizer(
        questions,
...
class QuestionAnsweringTrainer(Trainer):
    ...
    def evaluate(self,
        ...
        self.control = self.callback_handler.on_evaluate(self.args, self.state,
                                                         self.control, metrics) #type: ignore
        return metrics
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        train_ids = list(inputs.pop('train_ids').detach().cpu().numpy())

        outputs = model(**inputs)

        start_logits = list(outputs.get('start_logits').detach().cpu().numpy())
        start_positions = list(inputs.get('start_positions').detach().cpu().numpy())
        
        self.dynamics.aggregate(train_ids, start_logits, start_positions)
