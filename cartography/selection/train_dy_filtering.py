...
def write_filtered_data(args, train_dy_metrics):
    ...
    original_train_file = os.path.join(os.path.join(
        args.data_dir, train_sub_dir), train_sub_file)
...
def plot_data_map(dataframe: pd.DataFrame,
    ...
    if show_hist:
        ...
        plot2 = sns.countplot(x="correct.", data=dataframe.sort_values(
            "correct."), ax=ax3, color='#86bf91')
        u_ticks = [float(t) for t in sorted(
            dataframe["correct."].unique().tolist())]
        # only label the min, middle and max value
        plot2.set_xticklabels([t if (t == min(u_ticks) or t == max(
            u_ticks) or t == u_ticks[len(u_ticks)//2]) else "" for t in u_ticks])
